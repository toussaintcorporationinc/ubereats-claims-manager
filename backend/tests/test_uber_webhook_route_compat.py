...
def test_first_registered_uber_webhook_uses_live_event_handler(monkeypatch):
    captured = {}
    ...
    def fake_handle(self, db, payload):
        captured.update(payload)
        return {"status": "processed"}

    monkeypatch.setattr(UberConnectorService, "handle_webhook_event", fake_handle)
    monkeypatch.setattr(UberConnectorService, "process_webhook", fake_handle, raising=False)
