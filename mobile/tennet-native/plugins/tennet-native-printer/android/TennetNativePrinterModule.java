...
public class TennetNativePrinterModule extends ReactContextBaseJavaModule {
  ...
  private byte[] buildEscPosTicket(ReadableMap ticket) throws Exception {
    ...
    write(out, esc("@"));
    write(out, align(1));
    writeLine(out, restaurantName.isEmpty() ? "RESTAURANT" : restaurantName);
    writeLine(out, "COMMANDE UBER - PREUVE");
    writeLine(out, "");
    write(out, align(0));
    writeRule(out);
    ...
    writePair(out, "Reference preuve", ticketReference);
    writeRule(out);
    writeLine(out, title);
    writeLine(out, "1. Garder ce ticket avec la commande.");
    writeLine(out, "2. Photographier ticket + preuve.");
    writeLine(out, "3. Scanner le QR code pour l'envoyer.");
    writeRule(out);
    write(out, align(1));
